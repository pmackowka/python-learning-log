# Sprawdzanie warunków wstępnych (istnienie katalogów wejścia/wyjścia) przed przetwarzaniem.
import datetime
import os
...
is_today_output_catalog_ok = not (os.path.isdir(today_output_catalog)) and not (
    os.path.isfile(today_output_catalog)
)
...
if is_input_catalog_ok and is_output_catalog_ok and is_today_output_catalog_ok:
    ...
    if not is_today_output_catalog_ok:
        print(
            "Today's output %s cannot exist (neither as file nor as directory)!"
