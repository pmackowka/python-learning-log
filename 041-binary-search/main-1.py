...
szukana = 500  # zmienna przechowująca szukaną wartość
...
while lewy <= prawy:  # dopóki indeks lewy jest mniejszy lub równy prawemu
    srodkowy = (
        (lewy + prawy) // 2
    )  # wyliczamy indeks srodkowy, jako średnią indeksów lewy i prawy (// - zwraca część całkowitą z dzielenia)
    if (
        P[srodkowy] == szukana
    ):  # jeśli lista od inseksu srodkowy zawiera szukaną wartość
        print(
            f"Liczba {szukana} znajduje się na liście."
        )  # wypisz informację, że wartość znaleziono
        break  # i przerwij działanie pętli while
    elif (
        P[srodkowy] < szukana
    ):  # jeśli lista od indeksu srodkowy zawiera wartość mniejszą niż szukana
        lewy = (
            srodkowy + 1
        )  # przesuń indeks lewy na pozycję o jeden większą niz srodkowy
    else:  # w przeciwnym wypadku (czyli jeśli lista od indeku srodkowy zawiera wartość większa niż szukana)
        prawy = (
            srodkowy - 1
        )  # przesuń indeks prawy na pozycję o jeden mniejszą niż srodkowy
