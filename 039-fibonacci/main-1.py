...
def fibonacci_l(n):  # O(n)
    ...
    for _ in range(
        n
    ):  # _ - oznacza, że nie potrzebujemy zmiennej do przechowywania kolejnej wartości z range, chcemy tylko aby pętla wykonała się n razy
        p, d = (
            d,
            p + d,
        )  # jednoczesne aktualizowanie zawartości zmiennych p i d, aby przypisać do nich wartości z poprzedniego obiegu pętli
    return p
...
def fibonacci_r(n):  # O(2 ^ n)
    if (
        n <= 1
    ):  # każda funkcja rekurencyjna musi mieć warunek, który powoduje returna bez kolejnego wywołania rekurencji
    ...
    )  # dla każdego elementu innego niż zerowy lub pierwszy, funkcja wywołuje samą siebie
