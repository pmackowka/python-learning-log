...
def fizzbuzz(n):
    for num in range(1, n + 1):  # range do n+1 bo chcemy wypisać liczby do n włącznie
        if (
            num % 15 == 0
        ):  # w tej wersji zadania ważna jest kolejność bloków if, elif, else
            print(
                "FizzBuzz"
            )  # gdybyśmy jako pierwsze sprawdzili podzielność przez 3 lub 5
        elif (
            num % 3 == 0
        ):  # to program nigdy nie sprawdziłby podzielności przez 3 ORAZ 5
            print("Fizz")  # a więc nigdy nie wydrukowałby FizzBuzz
        elif (
            num % 5 == 0
        ):  # zatem warunek podzielności przez 3 ORAZ 5 musi być sprawdzony jako pierwszy
            print("Buzz")
        else:
            print(num)
...
def fizzbuzz(n):
    for num in range(1, n + 1):
        wynik = (
            ""  # w każdym obiegu pętli tworzymy pusty string do przechowywania wyniku
        )
        if num % 3 == 0:
