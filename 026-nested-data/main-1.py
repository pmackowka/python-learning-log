...
]  # tworzenie pięcioelementowej listy losowych integerów z zakresu od 0 do 100
...
for a in A:  # dla każdego elementu w liście A
    for b in B:  # dla każdego elementu w liście B
        for h in H:  # dla każdego elementu w liście H
            if (
                a * b * h > max_v
            ):  # 125 operacji  # jeśli a*b*h jest większe niż maksymalna znaleziona objętość
...
print(
    max(A) * max(B) * max(H)
)  # 15 operacji     # pomnóż przez siebie największe elementy list A, B i H
